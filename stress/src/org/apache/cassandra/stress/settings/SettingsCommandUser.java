...
public class SettingsCommandUser extends SettingsCommand
{
    ...
    public SettingsCommandUser(Options options, SettingsMode mode)
    {
        ...
        profile = StressProfile.load(URI.create(yamlPath), mode);
        ...
        return new SampledOpDistributionFactory<String>(ratios, clustering)
        {
            protected List<? extends Operation> get(Timer timer, PartitionGenerator generator, String key)
            {
                if (key.equalsIgnoreCase("insert"))
                	if (settings.mode.api == ConnectionAPI.JDBC) return Collections.singletonList(profile.getJdbcInsert(timer, generator, seeds, settings));
                	else return Collections.singletonList(profile.getInsert(timer, generator, seeds, settings));
    ...
    public static SettingsCommandUser build(String[] params, SettingsMode mode)
    {
        ...
        return new SettingsCommandUser((Options) options, mode);
    }

...
public class SettingsMode implements Serializable
{
    ...
    public SettingsMode(GroupedOptions options)
    {
        ...
        else if (options instanceof JDBCOptions)
        {
        	JDBCOptions opts = (JDBCOptions) options;
        	cqlVersion = CqlVersion.NOCQL;
        	api = ConnectionAPI.JDBC;
        	style = ConnectionStyle.JDBC;
        	compression = ProtocolOptions.Compression.NONE.name();
        	username = opts.user.value();
            password = opts.password.value();
            authProviderClassname = null;
    ...
    private static final class JDBCOptions extends GroupedOptions
    {
        final OptionSimple api = new OptionSimple("jdbc", "", null, "", true);
        final OptionSimple user = new OptionSimple("user=", ".+", null, "username", false);
        final OptionSimple password = new OptionSimple("password=", ".+", null, "password", false);
        ...
        public List<? extends Option> options()
        {
            return Arrays.asList(api, user, password);
        }
    }
    ...
    public static SettingsMode get(Map<String, String[]> clArgs)
    {
        ...
        GroupedOptions options = GroupedOptions.select(params, new ThriftOptions(), new Cql3NativeOptions(), new Cql3ThriftOptions(), new Cql3SimpleNativeOptions(), new Cql2ThriftOptions(), new JDBCOptions());
        if (options == null)
        {
            printHelp();
    ...
    {
        GroupedOptions.printOptions(System.out, "-mode", new ThriftOptions(), new Cql3NativeOptions(), new Cql3ThriftOptions(), new Cql3SimpleNativeOptions(), new Cql2ThriftOptions(), new JDBCOptions());
    }

...
public class StressSettings implements Serializable
{
    ...
    public final SettingsJDBC jdbcURL;

    public StressSettings(SettingsCommand command, SettingsRate rate, SettingsPopulation generate, SettingsInsert insert, SettingsColumn columns, SettingsSamples samples, SettingsErrors errors, SettingsLog log, SettingsMode mode, SettingsNode node, SettingsSchema schema, SettingsTransport transport, SettingsPort port, String sendToDaemon, SettingsJDBC jdbc)
    {
    ...
    private static volatile JdbcDriverClient jdbcClient;
    
    public JdbcDriverClient getJdbcDriverClient() {
    	if(jdbcClient != null)
    		return jdbcClient;
    	JdbcDriverClient client = new JdbcDriverClient(this);
    	try {
    		client.connect();
    	} catch(Exception e) {
    		System.err.println("Got Exception while establishing JDBC connection:" + e);
    	}
    	return client;
    }
    ...
    public static StressSettings get(Map<String, String[]> clArgs)
    {
        ...
        SettingsCommand command = SettingsCommand.get(clArgs, mode);
        if (command == null)
        ...
        SettingsJDBC jdbc = SettingsJDBC.get(clArgs);
        if (!clArgs.isEmpty())
        {
            printHelp();
        ...
        return new StressSettings(command, rate, generate, insert, columns, samples, errors, log, mode, node, schema, transport, port, sendToDaemon, jdbc);
    }

...
public class JDBCInserter extends CqlOperation<Integer>
{

    public JDBCInserter(Timer timer, PartitionGenerator generator, SeedManager seedManager, StressSettings settings)
    {
    ...
    {
        StringBuilder query = new StringBuilder("INSERT INTO " + type.table);
        ...
        for (int i = 0 ; i < settings.columns.names.size() ; i++)
        {
            ...
            query.append(settings.columns.namestrs.get(i));
        }
        query.append(", K) VALUES (?, ");
        for (int i = 0 ; i < settings.columns.names.size(); i++)
        {
            if (i > 0)
                query.append(',');

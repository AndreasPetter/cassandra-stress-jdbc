...
import org.apache.cassandra.stress.util.JdbcDriverClient.Either;
...
public abstract class CqlOperation<V> extends PredefinedOperation
{
    ...
    public ClientWrapper wrap(JdbcDriverClient client)
    {
        return new JDBCDriverWrapper(client);
    }
    ...
    private final class JDBCDriverWrapper implements ClientWrapper
    {
        final JdbcDriverClient ds;
        private JDBCDriverWrapper(JdbcDriverClient client)
        {
            this.ds = client;
        }
        ...
        {
        	JdbcDriverClient.Either<java.sql.ResultSet, Integer> rSet = null;
        	rSet = ds.execute(formatJDBCQuery(query, queryParams));
			return handler.jdbcHandler().apply(rSet);
        }
        ...
        {           
        	...
        	try {
        		...
        		java.sql.PreparedStatement prep = (java.sql.PreparedStatement)preparedStatementId;
        		for(int i = 1; i <= queryParams.size(); i ++) {
            		prep.setString(i, getBase64Blob(ByteBuffer.wrap(queryParams.get(i - 1).toString().getBytes("UTF-8"))));        			
        		}
        		prep.execute();
            	System.out.println("T2.2");
        		return handler.jdbcHandler().apply(Either.<java.sql.ResultSet, Integer>left(prep.getResultSet()));
				...
				try {
					...
					System.err.println("Could not return connection to the pool");
				}
			}
        }
        ...
        public Object createPreparedStatement(String cqlQuery)
        {
            ...
            try {
            	...
            	return connection.prepareStatement(cqlQuery);
            } catch(SQLException e) {
            	System.err.println("Could not prepare statement: " + cqlQuery);
            	return null;
            } finally {
				try {
					connection.close();
    ...
    protected static interface ResultHandler<V>
    {
        ...
        Function<Either<java.sql.ResultSet, Integer>, V> jdbcHandler();
    }
    ...
    protected static class RowCountHandler implements ResultHandler<Integer>
    {
        ...
        @Override
        public Function<Either<java.sql.ResultSet, Integer>, Integer> jdbcHandler()
        {
        	return new Function<Either<java.sql.ResultSet, Integer>, Integer>()
        	{
                @Override
                public Integer apply(Either<java.sql.ResultSet, Integer> result)
                {
                	int count = 0;
                	try {
                		if(result.isLeft()) {
                			java.sql.ResultSet rSet = result.getLeft();
                			while(rSet.next()) {
                				count += 1;
                			}
                		} else {
                			count = result.getRight();
                		}
                	} catch(SQLException e) {
                		// do nothing;
                	}
    ...
    protected static final class RowsHandler implements ResultHandler<ByteBuffer[][]>
    {
        ...
        @Override
        public Function<Either<java.sql.ResultSet, Integer>, ByteBuffer[][]> jdbcHandler()
        {
        	return new Function<Either<java.sql.ResultSet, Integer>, ByteBuffer[][]>()
        	{
                @Override
                public ByteBuffer[][] apply(Either<java.sql.ResultSet, Integer> result)
                {
                	ArrayList<ByteBuffer[]> bbs = new ArrayList<ByteBuffer[]>();
                	try {
                    	ResultSetMetaData rsmd = result.getLeft().getMetaData();
                    	int numberOfColumns = rsmd.getColumnCount();
	                	while(result.getLeft().next()) {
	                		ByteBuffer[] bb = new ByteBuffer[numberOfColumns];
	                		for(int i = 1; i <= numberOfColumns; i ++) {
		                		bb[i - 1] = ByteBuffer.wrap((byte[])result.getLeft().getObject(i));
	                		}
	                		bbs.add(bb);
	                	}
                	} catch(SQLException e) {
    ...
    protected static final class KeysHandler implements ResultHandler<byte[][]>
    {
        ...
        @Override
        public Function<Either<java.sql.ResultSet, Integer>, byte[][]> jdbcHandler()
        {
        	return new Function<Either<java.sql.ResultSet, Integer>, byte[][]>()
        	{
                @Override
                public byte[][] apply(Either<java.sql.ResultSet, Integer> result)
                {
                	ArrayList<byte[]> bbs = new ArrayList<byte[]>();
                	try {
	                	while(result.getLeft().next()) {
	                		bbs.add(result.getLeft().getBytes(1));
	                	}
                	} catch(SQLException e) {
                		// do nothing;
                	}
                    return (byte[][])bbs.toArray();
                }        		
        	};
        }
    }

    private static String getBase64Blob(ByteBuffer term)
    {
     ...
     * @return formatted CQL query string
     */
    private static String formatJDBCQuery(String query, List<Object> parms)
    {
        int marker, position = 0;
        StringBuilder result = new StringBuilder();
        ...
        for (Object parm : parms)
        {
            ...
            if (parm instanceof ByteBuffer) {
            	result.append("'");
                result.append(getBase64Blob((ByteBuffer) parm));
            	result.append("'");
            } else if (parm instanceof Long) {
                result.append(parm.toString());
            }

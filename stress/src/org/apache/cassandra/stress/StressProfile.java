...
import org.apache.cassandra.stress.operations.userdefined.JdbcSchemaInsert;
...
public class StressProfile implements Serializable
{
    ...
    transient volatile DatabaseMetaData jdbcMetaData;
    ...
    transient volatile String jdbcInsertQuery;
    transient volatile JdbcDriverClient jdbcClient;
    ...
    private void init(StressYaml yaml, SettingsMode mode) throws RequestValidationException
    {
        ...
        if (tableCql != null && tableCql.length() > 0)
        {
            try
            {
            	if(mode.api != ConnectionAPI.JDBC) {
    ...
    {
        if(settings.mode.api == ConnectionAPI.JDBC) {
        	
        	if (keyspaceCql != null) {
            	// we assume that the database has been created by the dbadmin beforehand, so we warn
        		System.err.println("WARN: Jdbc stress-driver does not create databases, only tables. Create this by hand!");
        	}
        	
        	if(tableCql != null) {
        		JdbcDriverClient client = settings.getJdbcDriverClient();
        		client.execute(tableCql);
        	}
    ...
    private void maybeLoadJdbcSchemaInfo(StressSettings settings)
    {
        if (jdbcMetaData == null)
        {
            ...
            synchronized (client)
            {
                ...
                try {
                	...
                	if(settings.jdbcURL.upper) {
                		tableName = tableName.toUpperCase();
                	}
                	ResultSet rSet = jdbcMetaData.getColumns(null, null, tableName, null);
                    if (rSet == null)
                	...
                	while(rSet.next()) {
                        ...
                        if (columnConfigs.containsKey(columnName)) {
                            continue;
                        }
                        System.out.println("Column" + columnName);
                        columnConfigs.put(columnName, new GeneratorConfig(seedStr + columnName, null, null, null));
                	}
                	rSet.close();
                } catch (SQLException e) {
                    throw new RuntimeException("Unable to retrieve columns from table " + tableName);
				} finally {
                	try {
						connection.close();
					} catch (SQLException e) {
						System.err.println("WARN: could not close connection while retrieving meta-data. Coninuing anyways.");
					}
                }
    ...
    public JdbcSchemaInsert getJdbcInsert(Timer timer, PartitionGenerator generator, SeedManager seedManager, StressSettings settings)
    {
    	if(jdbcInsertQuery == null)
    	{
    		synchronized (this)
    		{
    			if(jdbcInsertQuery == null)
    			{
                    jdbcClient = settings.getJdbcDriverClient();
                    List<String> columns = new ArrayList<String>();
                    Connection connection = jdbcClient.getConnection();
                    try {
                        DatabaseMetaData databaseMetaData = connection.getMetaData();
                        ResultSet rSet = databaseMetaData.getColumns(null, null, tableName, null);
                        while(rSet.next()) {
                        	columns.add(rSet.getString("COLUMN_NAME").toLowerCase());
                        }
                        rSet.close();
                    } catch(SQLException sqlE) {
                    	System.err.println("ERROR: Could not retrieve column names from the databse! " + sqlE);
                    	System.exit(1);
                    } finally {
                    	try {
                    		connection.close();
                    	} catch(SQLException sqlE) {
                    		System.err.println("WARNING: Could not close database connection using JDBC driver:" + sqlE);
                    	}
                    }
                    ...
                    jdbcInsertQuery = sb.toString();
    			}
    		}
    	}
    	try {
    		return new JdbcSchemaInsert(timer, settings, generator, seedManager, partitions.get(), selectchance.get(), jdbcClient, jdbcInsertQuery);
    	} catch(SQLException sqlE) {
    		System.err.println("Error: Could not create JdbcSchemaInsert statement:" + sqlE);
    		return null;
    	}
    }
    ...
    public PartitionGenerator newGenerator(StressSettings settings)
    {
        if (generatorFactory == null)
        {
            ...
            {
            	if(settings.mode.api == ConnectionAPI.JDBC) {
            		maybeLoadJdbcSchemaInfo(settings);
            	} else {
            		maybeLoadSchemaInfo(settings);
            	}
        		if (generatorFactory == null)
    ...
    private class GeneratorFactory
    {
        ...
        private GeneratorFactory(StressSettings settings)
        {
        	if(settings.mode.api != ConnectionAPI.JDBC) {
        		...
        		List<ColumnInfo> tmpValueColumnInfos = new ArrayList<>();
				try {
					ResultSet cSet = jdbcMetaData.getColumns(null, null, tableName, null);
	        		try {
	        			while(cSet.next()) {
	        				System.out.println("Column: " + cSet.getString("COLUMN_NAME").toLowerCase());
	        				tmpValueColumnInfos.add(new ColumnInfo(cSet.getString("COLUMN_NAME").toLowerCase(), transformJdbcToCassandraType(cSet.getInt("DATA_TYPE")), columnConfigs.get(cSet.getString("COLUMN_NAME").toLowerCase())));
	        			}
	        		} finally {
	        			cSet.close();
	        		}
	        		
	        		valueColumns.addAll(tmpValueColumnInfos);
	        		
	        		ResultSet rSet = jdbcMetaData.getPrimaryKeys(null, null, tableName);
	        		try {
	        			while(rSet.next()) {
	        				for(ColumnInfo cInfo: tmpValueColumnInfos) {
	        					if(cInfo.name == rSet.getString("COLUMN_NAME").toLowerCase()) {
	        						partitionKeys.add(cInfo);
	        						System.out.println("Partition Key:" + rSet.getString("COLUMN_NAME").toLowerCase());
	        						valueColumns.remove(cInfo);
	        					}
	        				}
	        			}
	        		} finally {
					...
					System.err.println("Could not retrieve meta-data for JDBC table.");
				}
        		
        	}
        }

        private DataType transformJdbcToCassandraType(int type) {
        	// TODO: these are not really accurate as JDBC types differ. How can we make this better?
        	if(type == Types.CHAR || 
    ...
    public static StressProfile load(URI file, SettingsMode mode) throws IOError
    {
        try
        {
